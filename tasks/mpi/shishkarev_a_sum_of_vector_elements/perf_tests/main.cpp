...
TEST(shishkarev_a_sum_of_vector_elements_mpi, test_pipeline_run) {
  ...
  auto MPIVectorSumParallel = std::make_shared<shishkarev_a_sum_of_vector_elements_mpi::MPIVectorSumParallel>(taskDataPar);
  ASSERT_EQ(MPIVectorSumParallel->validation(), true);
  ...
  auto perfAnalyzer = std::make_shared<ppc::core::Perf>(MPIVectorSumParallel);
